...
3. **The burst is replay commit granularity, not VFS overhead — and it matches the #1089 benchmark exactly.** The PR's measured steady-state is **12 writes/todo** (238→12, 95.0%); cloudstash replayed at 10.7–12 rows/event. The PR even states the consequence that became this incident: "At 12 writes per event, the CF free tier (100k rows_written/day) supports ~8,300 events/day" — the cutover materialized 2 DOs × 4,466 = **8,932 events in one hour**, consuming that daily budget almost to the digit. The PR's bulk benchmark (10 todos = 60 writes = 6/event) also empirically confirms the batching-amortization lever below. Pre-#1089 (64 KB blocks + `mergePartialBlock` + journal-on-VFS) the same replay would have been ~10–20× worse; the first DO alone would have blown the cap.
