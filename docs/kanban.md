...
- [ ] Retire the local Better Auth MCP JWKS verifier after [#10856](https://github.com/better-auth/better-auth/issues/10856) / [#10888](https://github.com/better-auth/better-auth/issues/10888) ship in a stable release — switch back to the high-level handler only after the same-Worker Cloudflare E2E passes with no public self-fetch.
- [ ] Retest Better Auth OAuth resource seeding on future stable upgrades — remove `auth/mcp-resource.ts` once concurrent fresh D1/Drizzle auth contexts succeed without the atomic pre-seed; no upstream report is being filed until the reproduction is independently confirmed.
