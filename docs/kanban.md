...
- [ ] [[architecture/sync-backend-do-hibernation-billing|Drop local DO-hibernation patches once LiveStore #1328 lands the upstream fix]] — the SyncBackendDO `Effect.never`-defeats-WS-hibernation fix is **shipped & prod-confirmed** (~1,300× idle-billing drop) but lives in local `patches/` and is **our own attempt, not a confirmed-correct approach**. First iteration was the global `effect` neuter ([cloudstash#78](https://github.com/bohdanbirdie/cloudstash/pull/78), now superseded); current is a surgical 3-site WS timer-less park + DO-RPC subscription persistence. Tracking upstream: [livestorejs/livestore#1328](https://github.com/livestorejs/livestore/issues/1328). **When it lands:** drop our patches, remove the temp `liveLongTimers` probe (`src/cf-worker/sync/index.ts`), re-validate (prod `type:hibernation` GB-s + sync e2e). Also still open: LinkProcessorDO client-side hibernation (deferred + unvalidated — needs a clean-store re-test, not a new patch). Full history: post-mortem doc.
