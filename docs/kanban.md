...
- [ ] [[todos/server-ingest-durability|Server-side ingest durability — links lost when DO backend is disabled]] — root cause verified 2026-06-11: server ingests (Telegram / Raycast / public `/api/ingest`) are queue→DO→SyncBackend, not local-first; when DOs are disabled (daily duration cap) the queue message retries×3 then dead-letters to `cloudstash-link-dlq`, which has **no consumer** → link silently lost, user gets false "saving"/`{queued}` and no reply. Web app + Chrome extension are SAFE (local-first OPFS livestore clients). Fix dir: D1 `pending_ingests` capture-before-ack + DLQ drain + carried idempotency key.
...
- [ ] [[todos/link-processor-stuck-after-eviction|LinkProcessor: self-heal after DO eviction]] — fixed upstream by #1544/#1545 (merged 2026-08-09: backend re-delivers + a reconstructed store-less client DO recovers live updates, closes livestorejs#1415), but only on the Effect-v4 codebase — lands here via [[todos/effect-v4-livestore-upstream-migration]]. A v3-fork backport is possible interim, only if this bites in prod before the swap.
- [ ] [[todos/admin-server-ahead-alert|Admin alert for stuck LinkProcessorDO sync (Telegram via Tail Worker)]]
- [ ] [[architecture/livestore-do-rpc-stream-stall|Upstream livestore PR — DO-RPC stream-framing fix (drain-then-decode + return runStream)]] — likely obsolete (2026-08-09): drain-then-decode merged upstream as #1167 (2026-04) and the DO-RPC/ws-rpc layer has been reworked wholesale since (Effect v4). After the [[todos/effect-v4-livestore-upstream-migration|upstream swap]], verify the stall no longer reproduces on upstream `main`, then close.
- [ ] [[architecture/sync-backend-do-hibernation-billing|DO-hibernation upstreaming COMPLETE — local cleanup remains]] — upstream `main` now carries the full fix chain: idle sync-DO hibernation ([livestorejs/livestore#1328](https://github.com/livestorejs/livestore/issues/1328) closed COMPLETED 2026-07-21, guarded by our test PRs #1427/#1435 over both WS and DO-RPC) + DO-RPC recovery across backend AND client reconstruction (#1541/#1542/#1544/#1545, last merged 2026-08-09). The local `patches/` were already superseded by the vendored fork (PR #79/#80); the fork itself is now fully superseded upstream too. Remaining: (1) remove the temp `liveLongTimers` probe (`src/cf-worker/sync/index.ts:23`) once prod `type:hibernation` GB-s are re-confirmed after the next deploy; (2) retire the fork via [[todos/effect-v4-livestore-upstream-migration]]. Still open: LinkProcessorDO client-side hibernation (deferred + unvalidated — needs a clean-store re-test, not a new patch). Full history: post-mortem doc.
- [ ] [[todos/e2e-do-sync-testing|E2E testing for DO-to-DO sync]] — **PR #46 merged 2026-08-10** (`e278fd1`): 12 miniflare tests over the ingest surface (cold/warm boot, duplicates, concurrent ingests, cross-org isolation, fetch trigger, real queue batches) + suite-wide teardown-flake fix (`disableConsoleIntercept: true`). Remaining scope: sync-arrival assertions (eventlog), hibernation recovery, cross-client WS pull, concurrency semantics — app-level flows (ingest → sync → UI) only; upstream carries the livestore-internal suites (#1427/#1435/#1541).
