...
- **Disconnect = remote revoke (2026-06-02).** Confirming disconnect best-effort calls `DELETE /api/connect/extension` (`ConnectClient.disconnect`, apiKey-authed) to delete the paired key row server-side, then clears local creds regardless of the network result. The web app's Integrations card stops showing the browser as connected immediately. Caveat: sync auth runs only at the WS handshake (`sync/validate-payload.ts`), so an already-open offscreen socket keeps syncing until its next reconnect — accepted as low-stakes. Server: `handleDisconnectRequest`/`handleExtensionDisconnect` in `src/cf-worker/connect/extension.ts`.
...
**Account info (replaces the entitlement endpoint).** The popup shows the connected user's identity: `GET /api/connect/extension/account` (`Authorization: Bearer <apiKey>`) returns `{ user: { name, image } }` only (no entitlement/tier — name + avatar URL, no email). Client: `lib/services/account-client.ts` (`AccountClient`), which returns a tagged `AccountResult`: a **401 means the key was revoked → `{ tag: "unauthorized" }`**, which the popup (`app.tsx`) treats as a logout signal (drops local creds → connect screen → offscreen store shuts down → saving stops); every other failure **fails open** (`{ tag: "ok", account: null }`, header just stays empty). Fetched async, non-blocking, so the save UI boots immediately. This is what closes the web-app-revoke loop: revoking the key from the Integrations card logs the extension out on the next popup open (auth otherwise only re-checks at the WS handshake). The popup renders the avatar with `referrerPolicy="no-referrer"` (Google's `lh3.googleusercontent.com` 403s on a referer) and an initials fallback; favicons load the same way. No manifest CSP/host-permission change is needed for these images (`img-src` is unrestricted; `host_permissions` only governs fetch/cookies).
