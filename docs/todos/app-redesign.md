...
- **Tri-state bulk tag picker (follow-up to phase 3b).** `BulkTagPicker` is now add-only with search + inline create + brand-aligned plain `#name` rows (mirrors `TagCombobox`); `tagColorStyles` removed from `TagCombobox` rows so both pickers look the same. Remaining: tri-state per tag across the selection (all/some/none) — clicking a tag with `all`-state removes from all, `none/some` adds to those that don't have it. Optional surface: per-row "on 3 of 5" caption.
...
- **Keyboard navigation in the list — preview-pane semantics.** Up/down arrow moves focus + selection through the list, detail pane updates as you go, Esc restores focus to the row that opened it. Done right: ARIA `listbox` / `option` semantics, no focus-ring after mouse click, scroll-into-view on nav, no page-scroll on arrow, focus-restoration on Esc, real screen-reader testing. **Known symptoms to fix (browser `:focus-visible` heuristic — click silently focuses, next keypress flips modality to keyboard and renders the ring):** (1) outline appears on a list row after clicking the row and then pressing any key; (2) outline appears on the right-pane detail container after clicking inside it and then pressing any key. **Prior attempts (2026-04-25, all reverted):** custom `aria-activedescendant` impl (functionally worked, no AT coverage); `react-aria` hooks (`useListBox` + `useOption`) — fought the standalone shape since the hooks expect a compound widget owning focus elsewhere, arrows wouldn't fire. **Next attempt should consider** `react-aria-components` (pre-wired `<ListBox>` / `<ListBoxItem>`, larger bundle but designed for standalone use), or commit to the custom impl with `@testing-library/react` + `jest-axe` coverage. Touches `link-list.tsx`, `link-list-item.tsx`, and the detail-view's `[` / `]` nav (which should be removed — arrows replace them).
...
- **Activity grid → list filtering (followup).** Cell click filters the list to that day's saves; multi-cell selection (drag or shift-click) filters to a date range. Surface a clear "clear filter" affordance. Decide whether range selection updates the URL (`/?from=…&to=…`) or stays client-only.
