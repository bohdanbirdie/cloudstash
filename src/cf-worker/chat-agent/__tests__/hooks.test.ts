...
describe("agentHooks (unknown agent class fail-closed)", () => {
  ...
  it.each(["unknown", "summary", "research", ""])(
    "rejects agent class '%s' with 404 and never reaches auth",
    async (className) => {
      const response = await agentHooks.onBeforeConnect(
        request,
        { className, name: "any-org" },
        stubEnv
      );
  ...
  it("uses the same handler for onBeforeRequest", async () => {
    const response = await agentHooks.onBeforeRequest(
      request,
      { className: "unknown", name: "any-org" },
      stubEnv
    );
    expect(response?.status).toBe(404);
  });
