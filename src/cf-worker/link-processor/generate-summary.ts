 ...
 */
export const generateSummary = ({ url, metadata, extractedContent, env }: GenerateSummaryParams) =>
  Effect.gen(function* () {
    let content: string
    ...
    if (extractedContent?.content) {
      ...
      if (metadata?.title) contentParts.push(`Title: ${metadata.title}`)
      if (metadata?.description) contentParts.push(`Description: ${metadata.description}`)
