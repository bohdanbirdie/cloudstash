...
import { generateLinksMarkdown, generatePlainLinks } from '@/lib/export-markdown'
...
export function ExportDialog({ open, onOpenChange, links, pageTitle }: ExportDialogProps) {
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = `${pageTitle.toLowerCase().replace(/\s+/g, '-')}-export.${extension}`
        ...
        >
          ...
          <TabsContent value='full' className='flex-1 min-h-0'>
            <div className='grid grid-cols-2 gap-4 h-full'>
              <div className='flex flex-col min-h-0'>
                <p className='text-xs text-muted-foreground mb-2'>Raw Markdown</p>
                <textarea
                  readOnly
          ...
          <TabsContent value='plain' className='flex-1 min-h-0'>
            <div className='flex flex-col h-full'>
              <p className='text-xs text-muted-foreground mb-2'>One URL per line</p>
              <textarea
                readOnly
                value={plainContent}
                className='flex-1 w-full p-3 text-xs font-mono bg-muted border border-input rounded-none resize-none focus:outline-none overflow-auto'
