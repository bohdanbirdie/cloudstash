import { CheckIcon, CopyIcon, TicketIcon, PlusIcon, TrashIcon } from 'lucide-react'
    ...
    <TabsContent value='invites' className='flex-1 flex flex-col min-h-0'>
      {newInviteCode && (
        <div className='mb-3 p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800'>
          ...
          <div className='flex items-center gap-2'>
            ...
            <Button size='sm' variant='outline' onClick={() => onCopyCode(newInviteCode)}>
              {copiedCode ? (
      ...
      <div className='mb-3'>
        <Button size='sm' onClick={onCreate} disabled={isCreating}>
          {isCreating ? <Spinner className='h-3.5 w-3.5' /> : <PlusIcon className='h-3.5 w-3.5' />}
          Create Invite
        </Button>
      </div>
            ...
            return (
              ...
              >
                <div className='min-w-0 flex-1'>
                  <div className='flex items-center gap-2'>
                    <code className='font-mono text-xs font-medium'>{invite.code}</code>
                    {status === 'available' && (
                      <Badge
                        variant='outline'
